...
class TransformerClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        # concatenate cls and nul tokens
        ...
        return self.final_linear(cls_output) # (B,21)

    def estimate(self, x):
        # For simplicity of use between classifier and regresser
        logits = self(x)
        probs = nn.Softmax()(logits)
        dots = probs * torch.arange(21)
        avgs = torch.sum(dots, dim=1) # Average found to be better than mode for Classifier models
        return avgs
    
class TransformerRegresser(nn.Module):
    def __init__(self, d_model = 256, n_heads = 16, d_ffwd = 512, n_layers = 8, activation=nn.functional.gelu):
        super().__init__()
        self.cls_token = nn.Parameter(torch.randn((1, 1, d_model)))
